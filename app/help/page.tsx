            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="border-b border-gray-200 pb-3 mb-3">
                  <h4 className="font-semibold text-gray-800 mb-2">📚 학급 및 명단 관리</h4>
                </div>
                <FAQItem
                ...
                />
                <FAQItem
                  question="여러 학급을 관리할 수 있나요?"
                  answer="네, 가능합니다. 좌측 사이드바의 '학급 관리' 탭에서 여러 학급을 만들고 관리할 수 있습니다. 각 학급은 독립적으로 관리되며, 자리표를 저장할 때 해당 학급과 연결됩니다."
                />
                <FAQItem
                  question="성별 정보를 입력하지 않아도 되나요?"
                  answer="성별 정보는 선택사항입니다. 입력하지 않아도 자리 배치는 가능하지만, 분단형 배치에서 '남녀 짝' 또는 '동성 짝' 옵션을 사용하려면 성별 정보가 필요합니다."
                />

                {/* 자리 배치 */}
                <div className="border-b border-gray-200 pb-3 mb-3 mt-6">
                  <h4 className="font-semibold text-gray-800 mb-2">🎯 자리 배치</h4>
                </div>
                <FAQItem
                ...
                />
                <FAQItem
                  question="모둠형(4인) 배치는 어떻게 사용하나요?"
                  answer="모둠형(4인) 배치는 자동으로 6×6 그리드로 설정되며, 좌석을 클릭하여 비활성화하여 모둠 구조를 만들 수 있습니다. 예를 들어, 2×2 형태로 4개의 좌석을 활성화하고 주변을 비활성화하면 4인 모둠이 됩니다."
                />

                {/* 지정구역 */}
                <div className="border-b border-gray-200 pb-3 mb-3 mt-6">
                  <h4 className="font-semibold text-gray-800 mb-2">📍 지정구역</h4>
                </div>
                <FAQItem
                  question="지정구역 배치는 어떻게 사용하나요?"
                  answer="'지정구역 관리'에서 구역을 만든 후, '좌석 할당' 버튼으로 해당 구역에 포함할 좌석을 선택합니다. 그 다음 '배정' 버튼으로 각 구역에 배치할 학생을 선택하면 됩니다. 지정구역에 배정된 학생은 해당 구역의 좌석에만 배치되며, 배정하지 않은 학생은 일반 좌석에 배치됩니다."
                />
                <FAQItem
                  question="지정구역에 학생을 배정하지 않으면 어떻게 되나요?"
                  answer="지정구역에 학생을 배정하지 않으면, 해당 구역의 좌석은 일반 좌석처럼 사용됩니다. 지정구역에 배정된 학생만 해당 구역에 배치되고, 나머지 학생들은 일반 좌석에 배치됩니다."
                />
                <FAQItem
                  question="지정구역을 삭제하거나 수정할 수 있나요?"
                  answer="지정구역 카드의 삭제 버튼을 클릭하면 구역을 삭제할 수 있습니다. 구역 이름과 색상은 언제든지 변경할 수 있으며, 좌석 할당과 학생 배정도 언제든지 수정할 수 있습니다."
                />

                {/* 수동 조작 */}
                <div className="border-b border-gray-200 pb-3 mb-3 mt-6">
                  <h4 className="font-semibold text-gray-800 mb-2">🔄 수동 조작</h4>
                </div>
                <FAQItem
                  question="특정 학생을 특정 자리에 고정하려면?"
                  answer="먼저 자동 배치를 실행한 후, 해당 학생을 원하는 자리로 수동으로 이동(Swap)시킨 다음 학생 카드의 자물쇠 아이콘을 클릭하여 고정하세요. 고정된 학생은 '배치 시작'을 다시 실행해도 자리가 유지됩니다."
                />

                {/* 저장 및 불러오기 */}
                <div className="border-b border-gray-200 pb-3 mb-3 mt-6">
                  <h4 className="font-semibold text-gray-800 mb-2">💾 저장 및 불러오기</h4>
                </div>
                <FAQItem
                  question="이전에 저장한 자리표를 불러오려면?"
                  answer="좌측 사이드바의 '저장 목록' 탭에서 원하는 자리표 카드를 클릭하면 불러올 수 있습니다. 저장된 자리표는 해당 학급과 연결되어 있으며, 불러오면 해당 학급의 학생 정보도 함께 불러와집니다."
                />

                {/* 인쇄 */}
                <div className="border-b border-gray-200 pb-3 mb-3 mt-6">
                  <h4 className="font-semibold text-gray-800 mb-2">🖨️ 인쇄</h4>
                </div>
                <FAQItem
                  question="인쇄할 때 교사용 보기와 학생용 보기의 차이는 무엇인가요?"
                  answer="'교사용 보기'는 교사가 칠판을 바라보는 방향으로 출력되며, '학생용 보기'는 학생이 칠판을 바라보는 방향으로 180도 회전되어 출력됩니다. 학생용 보기에서는 책상 이미지도 함께 회전되어 표시되지만, 학생 이름과 번호는 정상 방향으로 표시되어 교사가 읽을 수 있습니다."
                />

                {/* 로그인 및 데이터 */}
                <div className="border-b border-gray-200 pb-3 mb-3 mt-6">
                  <h4 className="font-semibold text-gray-800 mb-2">🔐 로그인 및 데이터</h4>
                </div>
                <FAQItem
                  question="로그인은 어떻게 하나요?"
                  answer="Google, 네이버, 카카오 계정으로 로그인할 수 있습니다. 처음 로그인 시 자동으로 계정이 생성되며, 이후에는 해당 계정으로 바로 로그인됩니다."
                />
                <FAQItem
                  question="데이터는 어디에 저장되나요?"
                  answer="모든 데이터는 Firebase에 저장되며, 로그인한 계정별로 독립적으로 관리됩니다. 학급 명단, 자리표, 지정구역 등 모든 정보는 클라우드에 저장되어 다른 기기에서도 접근할 수 있습니다."
                />
              </CardContent>
            </Card>
          </TabsContent>
